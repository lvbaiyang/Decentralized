...
def split_obs(obs):
    ...
    obss.append(torch.stack((obs[:, 2], obs[:, 11]), dim=1))
    obss.append(torch.stack((obs[:, 3], obs[:, 12]), dim=1))
    obss.append(torch.stack((obs[:, 4], obs[:, 13]), dim=1))
    obss.append(torch.stack((obs[:, 5], obs[:, 14]), dim=1))
    obss.append(torch.stack((obs[:, 6], obs[:, 15]), dim=1))
    obss.append(torch.stack((obs[:, 7], obs[:, 16]), dim=1))
    obss.append(torch.stack(
        (obs[:, 0], obs[:, 1], obs[:, 8], obs[:, 9], obs[:, 10], obs[:, -1]), dim=1))
    # obss.append(obs[:, 0: 1])
    # obss.append(obs[:, 1: 2])
    # obss.append(obs[:, 2: 3])
    # obss.append(obs[:, 3: 4])
    # obss.append(obs[:, 0: 11])
    # obss.append(obs[:, 11: 12])
    # obss.append(obs[:, [0, 2, 6]])
    # obss.append(obs[:, [1, 3, 7]])
    # obss.append(obs[:, [4, 5, 8, 9, 10, 11]])
    return obss
...
if __name__ == '__main__':
    ...
    algo = PPO(topo, obs_dims, act_dims, global_dim,
               split_obs, split_act, 1e-4, 0.999, 10000, 32)
    # algo.load()
    # rms = pickle.load(open("rms.pkl", "rb"))
    dump_rewards = []
    ep = 0
    while True:
        states = []
